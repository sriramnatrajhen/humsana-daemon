...
def verify_license() -> LicenseInfo:
    """License gating removed. Humsana is fully open source and 100% local.
    Live vs dry-run is controlled entirely by execution_mode in config.yaml.
    No network calls, no license server, no phoning home."""
    return LicenseInfo(valid=True, tier="open-source")
...
class HumsanaConfig:
    ...
    enable_slack_status: bool = False  # off by default (one-click Slack setup retired)
